...
impl fmt::Display for Ed25519SignerInfo {
...
pub struct Ed25519SecurityClient {
    client_configs: ClientContractConfigs,
}

impl WarpdriveClient for Ed25519SecurityClient {
...
impl Ed25519SecurityClient {
    ...
    pub async fn get_signer_weight(&self, key: [u8; 32]) -> Result<u64, SorobanHelperError> {
        let res = query(
            &self.client_configs,
            "get_signer_weight",
    ...
    pub async fn list_signers(&self) -> Result<Vec<Ed25519SignerInfo>, SorobanHelperError> {
...
pub(crate) fn decode_signer_info(val: &ScVal) -> Result<Ed25519SignerInfo, SorobanHelperError> {
    ...
    let key: [u8; 32] = bytes.as_slice().try_into().map_err(|_| unexpected(val))?;
    let ScVal::U64(weight) = weight_val else {
        return Err(unexpected(val));
    };
    Ok(Ed25519SignerInfo {
        key,
        weight: *weight,
    })
}
